...
class PngWriter {
  ...
  public:
    PngWriter(const int width, const int height) {
        ...
        // fill buffer with a "nice" cyan [73,175,205] -- eventually a designer
        // should choose this ;)
    ...
    void set(const int i, const int j, const unsigned char r, const unsigned char g, const unsigned char b) {
        ...
        // modify this set routine's passed j so that zero is at the bottom left...
    ...
    void write(char *filename) {
        ...
        png_set_IHDR(png_ptr, info_ptr, nx, ny, // width, height
                     8,                         // bits per pixel -- 16 does not work with blockbuster
                     PNG_COLOR_TYPE_RGB,        // non-alpha options are
                                                // PNG_COLOR_TYPE_RGB,PNG_COLOR_TYPE_GRAY,
                     PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);

        // Some bits per pixel notes: 16 does not work with blockbuster, and there
        // are also issues with PNG_COLOR_TYPE_GRAY interpretation, so stick to 8
        // and PNG_COLOR_TYPE_RGB for now. Note that if you do use 16, pay attention
        // to MSB/LSB order. Endian is flipped on my linux workstation...
        ...
        // leave the image data in buffer to allow the calling process to change it
        // and/or write another image. It is deleted in the destructor.
    }
};
